...
// This guest is only meaningful compiled to WebAssembly. Building it for the host
// target (e.g. `cargo clippy --workspace --all-targets`) would try to link a
// cdylib with an undefined `kelvane::infer` import — harmless on Linux, but a
// hard link error on macOS/Windows. Gate the guest code to `wasm32` so the host
// build produces an empty, link-clean library on every platform.
#[cfg(target_arch = "wasm32")]
mod guest {
    use kelvane_sdk::export_module;
    use serde_json::json;

    /// Upper bound on the number of action scores this module reads back. The
    /// guest is shape-agnostic — it forwards however many features it is given
    /// and accepts up to this many scores — so one module works across models of
    /// different input and output sizes (the host owns the actual shape).
    const MAX_ACTIONS: usize = 64;

    #[link(wasm_import_module = "kelvane")]
    extern "C" {
        /// Host inference call: reads `in_len` f32 at `in_ptr`, runs the loaded
        /// model, writes up to `out_cap` f32 scores at `out_ptr`. Returns the
        /// number of scores written, or a negative value on error.
    ...
    fn handle(input: &[u8]) -> Vec<u8> {
        ...
        // Forward the feature vector as-is; its length must match the loaded
        // model's input size, which the host enforces (a mismatch is an error).
